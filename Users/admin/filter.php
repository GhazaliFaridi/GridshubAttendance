          ...
          <div class="col-lg-3 col-xs-6">
            ...
            <div class="small-box bg-green">
              <div class="inner">
                ...
                $first_day_this_month = date('Y-m-01');
                $last_day_this_month  = date('Y-m-t');
                $data = "  SELECT * FROM users_data WHERE ac_no = " . $_SESSION['user_id'] . " AND dates BETWEEN '$first_day_this_month' AND '$last_day_this_month'";
                if ($row = mysqli_query($conn, $data)) {
                  if (mysqli_num_rows($row) > 0) {
                    $total = $row->fetch_assoc();

                    $clock_in = $total['clock_in'];
                    $clock_out = $total['clock_out'];
                    $time = $total['total_time'];

                    $percentage = (int)$clock_in + (int)$clock_out - (int)$time;
                    $add = number_format($percentage) * 8;
                                   ...
                                   <div class="box">
                                        ...
                                        <div class="box-body">
                                             ...
                                             if (isset($_POST["from_date"], $_POST["to_date"])) {
                                                  $data = "  SELECT * FROM users_data WHERE dates BETWEEN '{$_POST["from_date"]}' AND '{$_POST["to_date"]}' AND ac_no = " . $_SESSION['user_id'] . " GROUP BY dates ASC ";
